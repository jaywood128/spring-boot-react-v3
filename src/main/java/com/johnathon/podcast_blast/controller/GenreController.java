...
public class GenreController {
  ...
  public ResponseEntity<List<?>> getAllGenres(EpisodeRepository genreRepository) {
    ...
    JSONObject jsonObjects = webClientBuilder.defaultHeader(HttpHeaders.CONTENT_TYPE, MediaType.APPLICATION_JSON_VALUE)
        .defaultHeader("X-ListenAPI-Key", webSecurityConfig.getApiKey()).build().get()
        .uri(baseURL + "genres?top_level_only=0").retrieve().bodyToMono(JSONObject.class).block();
  ...
  @GetMapping(value = "/best-podcasts/{genreId}", produces = "application/json")
  ResponseEntity<List<JSONObject>> getTopPodcastsBasedOnGenreId(@PathVariable("genreId") String genreId) {

    List<JSONObject> bestPodcasts = new ArrayList<JSONObject>();

    JSONObject jsonObjects = webClientBuilder.defaultHeader(HttpHeaders.CONTENT_TYPE, MediaType.APPLICATION_JSON_VALUE)
        .defaultHeader("X-ListenAPI-Key", webSecurityConfig.getApiKey()).build().get()
        .uri(baseURL + "/best_podcasts?genre_id=" + genreId + "&region=us&safe_mode=0").retrieve()
        .bodyToMono(JSONObject.class).block();
    bestPodcasts.add(jsonObjects);
    if (bestPodcasts.size() != 0) {
      Iterator<Map.Entry<String, Object>> it = jsonObjects.entrySet().iterator();
    ...
    return new ResponseEntity<>(bestPodcasts, HttpStatus.OK);
